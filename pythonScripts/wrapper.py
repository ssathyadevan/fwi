...
run_fwi_cpp = subprocess.Popen('bin/FWI_PreProcess ' + caseFolder, shell=True)
# Wait for that process to finish
run_fwi_cpp.wait()

# Run the Processing executable
run_fwi_cpp = subprocess.Popen('bin/FWI_Process ' + caseFolder, shell=True)
# Wait for that process to finish
run_fwi_cpp.wait()

# Run the image visualizer from here. It can run independently
run_visualizer = subprocess.check_call(
    ['python', 'postProcessing-python3.py', caseFolder])
sys.exit()
