...
public:
    InversionConcrete_gpu(const InversionConcrete_gpu<T,volComplexField,volField,Greens,Frequencies>&) = delete;
    ...
    void createTotalField(const int &rank)
    {
        ...
        for (int i=0; i<m_nfreq; i++)
        {
            ...
            calcField<T,volComplexField,volField,Greens>(*m_greens[i], m_chi, p_0[i], p_tot[i], rank, m_nsrc);
