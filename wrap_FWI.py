...
run_fwi_cpp = subprocess.Popen('./try_qt')                      # Run the Cpp core of FWI. It can run independently
run_fwi_cpp.wait()                                              # Wait for that process to finish
os.chdir('../')                                                 # Go back to the original directory
run_visualizer = subprocess.call(['python', 'imageCreator.py']) # Run the visualizer from here. It can run independently
