...
PressureFieldSerial ConjugateGradientInversion::Reconstruct(const std::vector<std::complex<double>> &pData, GenericInput gInput)
{
    ...
    const int nSignals = _freq.nFreq * _src.nSrc * _recv.nRecv;
    double eta = 1.0 / (normSq(pData, nSignals));
    
    PressureFieldSerial chiEstimateOld(_grid);
    chiEstimateOld.Zero(); // maybe always do this in the constructor
    std::vector<std::complex<double>>& residuals = _forwardModel->calculateResidual(chiEstimateOld, pData);
