...
parser.add_argument("-rpp", "--refpreprocess",
                    help="Path of reference PreProcess application", default=defaultpreprocesspath)
parser.add_argument("-rp", "--refprocess",
                    help="Path of reference Process application", default=defaultprocesspath)
...
ref_CPU_time = findCPUtime("CPU time: ", testcase +
                           "/output/" + testcase + "PreProcess.out")
new_CPU_time = findCPUtime(
    "CPU time: ", testcasenew + "/output/" + testcasenew + "PreProcess.out")

datetime_bench_start = findWallClocktime(
    "Starting at ", testcase + "/output/" + testcase + "PreProcess.out")
datetime_bench_finish = findWallClocktime(
    "Finished at ", testcase + "/output/" + testcase + "PreProcess.out")
bench_total_seconds = (datetime_bench_finish - datetime_bench_start).seconds
datetime_new_start = findWallClocktime(
    "Starting at ", testcasenew + "/output/" + testcasenew + "PreProcess.out")
datetime_new_finish = findWallClocktime(
    "Finished at ", testcasenew + "/output/" + testcasenew + "PreProcess.out")
new_total_seconds = (datetime_new_finish - datetime_new_start).seconds

print("Wall clock time it took to do reference run:      %0.0i seconds" %
      bench_total_seconds)
print("Wall clock time it took to do new run:            %0.0i seconds" %
      new_total_seconds)
print("Speedup of new run compared to reference run:     %0.2f" %
      (float(bench_total_seconds)/float(new_total_seconds)))
print("CPU time it took to do reference run:             %0.1f seconds" %
      ref_CPU_time)
print("CPU time it took to do new run:                   %0.1f seconds" %
      new_CPU_time)


print("")
print("############################# OUTPUT COMPARISON #################################")
ref_preprocess_csv = testcase + "/output/" + \
    testcase + "InvertedChiToPressure.txt"
new_preprocess_csv = testcasenew + "/output/" + \
    testcasenew + "InvertedChiToPressure.txt"
...
ref_preprocess_array = ref_preprocess_array[...,
                                            0] + 1j * ref_preprocess_array[..., 1]
new_preprocess_array = new_preprocess_array[...,
                                            0] + 1j * new_preprocess_array[..., 1]
...
preprocess_rmse = numpy.sqrt(numpy.square(
    numpy.absolute(diff_preprocess_array)).mean())
...
print("Relative Inf-norm w.r.t. ref:                     %0.2f" %
      preprocess_infnorm)
...
run_ref_process = subprocess.Popen(refprocess +
                                   " " + testcase, shell=True, stdout=subprocess.PIPE)
run_ref_process.wait()


print("Running: " + newprocess + " " + testcase)
run_new_process = subprocess.Popen(newprocess +
                                   " " + testcasenew, shell=True, stdout=subprocess.PIPE)
...
ref_CPU_time = findCPUtime("CPU time: ", testcase +
                           "/output/" + testcase + "Process.out")
new_CPU_time = findCPUtime(
    "CPU time: ", testcasenew + "/output/" + testcasenew + "Process.out")

datetime_bench_start = findWallClocktime(
    "Starting at ", testcase + "/output/" + testcase + "Process.out")
datetime_bench_finish = findWallClocktime(
    "Finished at ", testcase + "/output/" + testcase + "Process.out")
bench_total_seconds = (datetime_bench_finish - datetime_bench_start).seconds
datetime_new_start = findWallClocktime(
    "Starting at ", testcasenew + "/output/" + testcasenew + "Process.out")
datetime_new_finish = findWallClocktime(
    "Finished at ", testcasenew + "/output/" + testcasenew + "Process.out")
new_total_seconds = (datetime_new_finish - datetime_new_start).seconds
...
numerics_filename_per_ref = testcasenew + \
    "/diff_chi_perfect_and_ref_"+testcase+".csv"
numerics_filename_per_new = testcasenew + \
    "/diff_chi_perfect_and_new_"+testcase+".csv"
numerics_filename_ref_new = testcasenew + \
    "/diff_chi_ref_and_new_"+testcase+".csv"
numpy.savetxt(
...
imagefilename = testcasenew+"/ref_new_"+testcase + \
    "_compare.png"  # try to save in NEW folder
