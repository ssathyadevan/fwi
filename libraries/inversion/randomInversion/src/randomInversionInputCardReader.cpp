...
void RandomInversionInputCardReader::readJsonFile(const std::string &filePath)
{
    ...
    double toleranceOuter = ReadJsonHelper::tryGetParameterFromJson<double>(jsonFile, _fileName, parameterToleranceOuter);
    if(toleranceOuter <= 0)
    {
    ...
    int maxIterationsInner = ReadJsonHelper::tryGetParameterFromJson<int>(jsonFile, _fileName, parameterMaxIterationsInner);
    ...
    int maxIterationsOuter = ReadJsonHelper::tryGetParameterFromJson<int>(jsonFile, _fileName, parameterMaxIterationsOuter);
