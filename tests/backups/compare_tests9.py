...
resolution_of_reservoir=max_chi_of_reference_input_reservoir-min_chi_of_reference_input_reservoir
nxcols = resultx.shape[0]
nxrows = resultx.shape[1]
print ""
print " The difference between the max and min value in your INPUT reservoir chi is: " +str(resolution_of_reservoir) 
...
resolution_of_bench=numpy.max(resultsy)-numpy.min(resultsy)
ynew = list(read_new)
resultsynew = numpy.array(ynew).astype("float")
z                  = numpy.empty([nxcols,nxrows], dtype=float)
diff_new_and_bench = numpy.empty([nxcols,nxrows], dtype=float)
...
for i in range (0,resultx.shape[0]): #-1):
    for j in range(0, resultx.shape[1]): #-1):
        ...
        z[i,j] = float_formatter ( (resultsy[i,j] - resultx[i,j])/resolution_of_reservoir*100 )
        diff_new_and_bench[i,j] = float_formatter ( (resultsynew[i,j] - resultsy[i,j])/resolution_of_bench*100 )
...
plt.imshow(z)
plt.colorbar()
#imgsave = plt.imsave("hi",z,format="png")
#plt.subplot(2,2,2)
plt.subplot(2,2,2)
