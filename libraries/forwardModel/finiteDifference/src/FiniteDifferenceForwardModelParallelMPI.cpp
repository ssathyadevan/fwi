...
void FiniteDifferenceForwardModelMPI::createP0() {
  ...
  for (int i = 0; i < _freq.count; i++) {
    ...
    for (int j = 0; j < _source.count; j++) {
      ...
      *_p0[i][j] = *(_Greens[i]->getReceiverCont(j)) /
                   (_freq.k[i] * _freq.k[i] * _grid.getCellVolume());
    }
  }
}
...
void FiniteDifferenceForwardModelMPI::createGreens() {
  ...
  for (int i = 0; i < _freq.count; i++) {
    _Greens[i] =
        new core::greensRect2DCpu(_grid, core::greensFunctions::Helmholtz2D,
                                  _source, _receiver, _freq.k[i]);
  }
}
...
void FiniteDifferenceForwardModelMPI::createPTot(
    const core::FrequenciesGroup &freq, const core::Sources &source) {
...
void FiniteDifferenceForwardModelMPI::createKappa(
    const core::FrequenciesGroup &freq, const core::Sources &source,
    const core::Receivers &receiver) {
  _kappa =
      new core::complexDataGrid2D *[freq.count * source.count * receiver.count];
...
void FiniteDifferenceForwardModelMPI::calculatePTot(
    const core::dataGrid2D &chiEst) {
  assert(_Greens != nullptr);
  assert(_p0 != nullptr);
  ...
  for (int i = 0; i < _freq.count; i++) {
    ...
    Helmholtz2D helmholtzFreq(_grid, _freq.freq[i], _source, _freq.c0, chiEst,
                              _fMInput);
...
FiniteDifferenceForwardModelMPI::calculatePressureField(
    const core::dataGrid2D &chiEst) {
  std::vector<std::complex<double>> kOperator(_freq.count * _source.count *
                                              _receiver.count);
  ...
  for (int i = 0; i < _freq.count; i++) {
    ...
    for (int j = 0; j < _receiver.count; j++) {
      ...
      for (int k = 0; k < _source.count; k++) {
        *_kappa[li + lj + k] =
            (*_Greens[i]->getReceiverCont(j)) * (*_pTot[i * _source.count + k]);
      }
    }
  }
}

void FiniteDifferenceForwardModelMPI::applyKappa(
    const core::dataGrid2D &CurrentPressureFieldSerial,
    std::vector<std::complex<double>> &kOperator) {
  for (int i = 0; i < _freq.count * _source.count * _receiver.count; i++) {
    kOperator[i] = dotProduct(*_kappa[i], CurrentPressureFieldSerial);
  }
}

void determineStartStopMPI(int &start, int &stop, const int &freq, int rank) {
