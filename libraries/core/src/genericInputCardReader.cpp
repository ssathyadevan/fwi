...
{
    if (_input.c_0 <= 0 ){throw std::invalid_argument("Invalid value for c_0.");}
    if (_input.freq.min > _input.freq.max ) {throw std::invalid_argument("Invalid ranges for frequenties");}
    if (_input.freq.nTotal <= 0 ) {throw std::invalid_argument("Invalid number of frequenties");}
    if (_input.reservoirTopLeftCornerInM[0] > _input.reservoirBottomRightCornerInM[0] || _input.reservoirTopLeftCornerInM[1] > _input.reservoirBottomRightCornerInM[1]) {throw std::invalid_argument("Invalid ranges for reservoir.");}
    if (_input.receiversTopLeftCornerInM[0] > _input.receiversBottomRightCornerInM[0] || _input.receiversTopLeftCornerInM[1] > _input.receiversBottomRightCornerInM[1]) {throw std::invalid_argument("Invald ranges for reseivers.");}
    if (_input.sourcesTopLeftCornerInM[0] > _input.sourcesBottomRightCornerInM[0] || _input.sourcesTopLeftCornerInM[1] > _input.sourcesBottomRightCornerInM[1]) {throw std::invalid_argument("Invalid ranges for reservoir.");}
    if (_input.ngrid_original[0] <= 0 || _input.ngrid_original[1] <= 0) {throw std::invalid_argument("Invalid grid_original input");}
    if (_input.ngrid[0] <= 0 || _input.ngrid[1] <= 0) {throw std::invalid_argument("Invalid grid input");}
    if (_input.nSourcesReceivers.nreceivers <= 0) {throw std::invalid_argument("Invalid number of receivers");}
    if (_input.nSourcesReceivers.nsources <= 0) {throw std::invalid_argument("Invalid number of sources");}
}
