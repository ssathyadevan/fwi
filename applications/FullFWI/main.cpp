...
void doPreprocess(const FullFWIOptions& fwiOpts, const fwi::io::genericInput& gInput);
void doProcess(const FullFWIOptions& fwiOpts, const fwi::io::genericInput& gInput);
void doPostprocess(const std::string& caseFolder);
...
void executeFullFWI(const FullFWIOptions& fwiOpts)
{
    ...
    if(!fwiOpts.skippost)
        doPostprocess(gInput.caseFolder);
    else
...
void doPostprocess(const std::string& caseFolder)
{
    std::cout << "Starting postprocessing..." << std::endl;
        ...
        const std::string cmd = "py -3 " + caseFolder + "/" + postScript + " -o " + caseFolder;
        if(system(cmd.c_str()))
        {
            std::cout << "Postprocessing failed" << std::endl
                      << "Make sure to place the file '" << postScript << "' in the case folder " << caseFolder << std::endl
