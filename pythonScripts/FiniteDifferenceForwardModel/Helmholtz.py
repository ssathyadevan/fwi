...
def solveHelmholtz(ABCorder):
    ...
    # x,y discretisation
    A = sparse.kron(sparse.eye(ny), xdirection) + \
        sparse.kron(ydirection, sparse.eye(nx))
    ...
    for i in range(0, nx):
        for j in range(0, ny):
            if (f(i, j) != 0.0):
                ...
                z0 = omega * n(i, j) * \
                    np.sqrt(np.square(xgrid-x0)+np.square(ygrid-y0))
...
def solvePML():
    ...
    for i in np.arange(0, nxl):
        for j in np.arange(0, nyl):
            ...
            s1 = Sx(x)
            s2 = Sy(y)
            A[idx1, idx1] = -2.*(s2/s1)/(hx*hx) - 2.*(s1/s2) / \
                (hy*hy) + s1*s2*n(i, j)*n(i, j)*omega*omega
...
print("")
print("2-Norm diff exact 1st:   %0.2f" %
      (np.linalg.norm(diff_exact_u) / np.linalg.norm(exactsol)))
print("2-Norm diff exact 2nd:   %0.2f" %
      (np.linalg.norm(diff_exact_v) / np.linalg.norm(exactsoltemp)))
print("2-Norm diff exact PML:   %0.2f" %
      (np.linalg.norm(diff_exact_w) / np.linalg.norm(exactsoltemp)))

print("")
print("1-Norm diff exact 1st:   %0.2f" %
      (np.linalg.norm(diff_exact_u, ord=1) / np.linalg.norm(exactsoltemp, ord=1)))
print("1-Norm diff exact 2nd:   %0.2f" %
      (np.linalg.norm(diff_exact_v, ord=1) / np.linalg.norm(exactsoltemp, ord=1)))
print("1-Norm diff exact PML:   %0.2f" %
      (np.linalg.norm(diff_exact_w, ord=1) / np.linalg.norm(exactsoltemp, ord=1)))
