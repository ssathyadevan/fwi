...
{
    return (r == 0.0 ? 0.0 : 0.25 / (r * fwi::pi) * std::cos(k * r) * k * k); // Babak 2018 10 25: Equation ID "GreensFunc2d": real pardouble of Greens function from a 3D Helmhotz equation.
}

double Helmholtz3D_imag(double k, double r)
{
    return (r == 0.0 ? 0.0 : 0.25 / (r * fwi::pi) * std::sin(k * r) * k * k); // Babak 2018 10 25: Equation ID "GreensFunc2d": imnaginary pardouble of Greens function from a 3D Helmhotz equation
}
