...
if (len(sys.argv) != 4):
    print("please supply four arguments after python compare_tests.py\n\
the first argument the benchmark, the second argument the new test\n")
    sys.exit()
else:
    bench   = sys.argv[1]
    new     = sys.argv[2]
    forwardmodel = sys.argv[3]
...
inputfiles = ["StepAndDirectionInput.json", forwardinputfile, "GenericInput.json"]
