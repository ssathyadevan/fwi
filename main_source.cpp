...
int main(int argc, char** argv)
{
    ...
    int ret =generateRefrencePressureFieldFromChi(fileName,// nxt, nzt
		                                  nSrct, input.freq,
						  // nFreq_Total, Freq_min, Freq_max,
...
int generateRefrencePressureFieldFromChi (const std::string &fileName,//const int& nxt, const int& nzt, 
		                          const int& nSrct,
                                          //const int& nFreq_Total, const double& Freq_min, const double& Freq_max,
                                          ...
                                          bool calc_alpha, int n_iter2, std::string runName, std::array<int,2> ngrid2)
{
    ...
    double d_freq = (input.freq.max - input.freq.min)/(input.freq.nTotal  -1);

    int sum = 0;


    f_min = input.freq.min + sum*d_freq;
    d_freq_proc = d_freq;

    Frequencies_group freq(f_min, d_freq_proc, input.nfreq.nTotal, c_0);
    freq.Print();

    ProfileInterface *profiler;
    profiler = new ProfileCpu();

    std::complex<double> referencePressureData[input.nFreq.nTotal * nRecv * nSrct];
