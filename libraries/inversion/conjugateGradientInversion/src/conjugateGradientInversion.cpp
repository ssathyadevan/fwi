...
/**
 * @brief ConjugateGradientInversion::Reconstruct This function iteratively reconstructs the contrast function of the
 ...
 * @return PressureFieldSerial result, the optimized estimation for the contrast function.
 */
PressureFieldSerial ConjugateGradientInversion::Reconstruct(const std::vector<std::complex<double>> &pData, GenericInput gInput)
{
    ...
    for (int it = 0; it < _cgInput.n_max; it++)
    {
        ...
        regularisationPrevious.fRegularisation = 1.0;

        // First iteration
        //------------------------------------------------------------------
        zeta = calculateUpdateDirection(residualArray, gradientCurrent, eta);                                                           // eq: updateDirectionsCG
...
{
    regularisationCurrent.bSquared = (regularisationPrevious.gradientChiNormSquared + regularisationPrevious.deltaSquared); // eq: errorFuncRegulWeighting
...
PressureFieldSerial ConjugateGradientInversion::calculateUpdateDirection_regularisation(std::vector<std::complex<double>> &residualArray, PressureFieldSerial &gradientCurrent, const PressureFieldSerial &gradientPrevious, const double eta, const RegularisationParameters &regularisationCurrent, const RegularisationParameters &regularisationPrevious, PressureFieldSerial &zeta, double residualPrevious)
{
    ...
    _forwardModel->getUpdateDirectionInformation(residualArray, kappaTimesResidual);
    gradientCurrent = eta * regularisationPrevious.fRegularisation * kappaTimesResidual.GetRealPart() + residualPrevious * regularisationCurrent.gRegularisation;          // eq: 2.25
    double gamma = gradientCurrent.InnerProduct(gradientCurrent - gradientPrevious) / gradientPrevious.InnerProduct(gradientPrevious);  // eq: PolakRibiereDirection
...
double ConjugateGradientInversion::calculateStepSize_regularisation(const RegularisationParameters &regularisationPrevious, RegularisationParameters &regularisationCurrent, const int nTotal, const std::vector<std::complex<double>> &resArray, const double eta, const double fDataOld, const PressureFieldSerial& zeta)
{
    ...
    {
        A[1] += eta * std::real(conj(kappaTimesZeta[i]) * kappaTimesZeta[i]);
        A[0] += double(-2.0) * eta * std::real(conj(resArray[i]) * kappaTimesZeta[i]);
    }
    ...
    PressureFieldSerial tmpVolField = regularisationCurrent.b * gradientZetaTmp[0];
    PressureFieldSerial tmpVolField2 = regularisationCurrent.b * gradientZetaTmp[1];
    tmpVolField.Square();
    tmpVolField2.Square();
    double B2 = (tmpVolField.Summation() + tmpVolField2.Summation()) * _grid.GetCellVolume();

    tmpVolField = (regularisationCurrent.b * gradientZetaTmp[0]) * (regularisationCurrent.b * regularisationPrevious.gradientChi[0]);
    tmpVolField2 = (regularisationCurrent.b * gradientZetaTmp[1]) * (regularisationCurrent.b * regularisationPrevious.gradientChi[1]);
    double B1 = double(2.0) * (tmpVolField.Summation() + tmpVolField2.Summation()) *
                _grid.GetCellVolume();

    tmpVolField = (regularisationCurrent.b * regularisationPrevious.gradientChi[0]) * (regularisationCurrent.b * regularisationPrevious.gradientChi[0]);
    tmpVolField2 = (regularisationCurrent.b * regularisationPrevious.gradientChi[1]) * (regularisationCurrent.b * regularisationPrevious.gradientChi[1]);
    double B0 = ((tmpVolField.Summation() + tmpVolField2.Summation()) + regularisationPrevious.deltaSquared *
                                                                            regularisationCurrent.bSquared.Summation()) *
                _grid.GetCellVolume();
...
void ConjugateGradientInversion::calculateRegularisationErrorFunctional(RegularisationParameters &regularisationPrevious, RegularisationParameters &regularisationCurrent)
{
    ...
    PressureFieldSerial tmpVolField = (gradientChiNormsquaredCurrent + regularisationPrevious.deltaSquared) / (regularisationPrevious.gradientChiNormSquared + regularisationPrevious.deltaSquared);
    regularisationPrevious.fRegularisation = (double(1.0) / (_grid.GetDomainArea())) *
