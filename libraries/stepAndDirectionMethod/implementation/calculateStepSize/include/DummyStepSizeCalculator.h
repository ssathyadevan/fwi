...
namespace fwi
{
    namespace inversionMethods
    {
        ...
        public:
             ...
             * @return is the StepSize optimized following the chosen algorithm implementation and the quantities that have been updated through
             * updateVariables()
             */
            double calculateStepSize();

            /**
             * @brief updateVariables takes a number of variables from the method StepAndDirectionReconstructor::Reconstruct and updates the variables that will
             * be needed by calculateStepSize(). If some variables are not needed, their names (but not the types) should be removed from the input in the
             * declaration (in this file) and the definition (in the .cpp file) and remove the (void)variableName as well.
