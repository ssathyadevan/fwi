...
void generateReferencePressureFieldFromChi(const io::genericInput &gInput, const std::string &runName)
{
    ...
    L_(io::linfo) << "Calculating pData (the reference pressure-field)...";
    ...
    L_(io::linfo) << "PData has been calculated in " << double(tEndCalculatePData - tStartCalculatePData) / CLOCKS_PER_SEC << "seconds.";
    ...
    if(!file)
    {
        L_(io::lerror) << "Failed to open the file to store inverted chi to pressure field" << std::endl;
        std::exit(EXIT_FAILURE);
    }
