...
def createCompareImages():
    ...
    print(results_df)
    lcpu, lcase = zip(*sorted(zip(results_df.CPU, results_df.CASE)))
    cpu_plot = plt.plot(lcase, lcpu)
    plt.xlabel("case")
    plt.ylabel("cpu_time")
    plt.savefig((path + "CPUImage.png"), dpi=400, bbox_inches='tight')
    # plt.show()
    plt.clf()
    lWall, lcase = zip(*sorted(zip(results_df.WALL, results_df.CASE)))
    wall_plot = plt.plot(lcase, lWall)
    plt.xlabel("case")
    plt.ylabel("wall_time")
    plt.savefig((path + "WALLImage.png"), dpi=400, bbox_inches='tight')
    # plt.show()
    plt.clf()
    ltime, lcase = zip(*sorted(zip(results_df.TIME, results_df.CASE)))
    time_plot = plt.plot(lcase, ltime)
    plt.xlabel("case")
    plt.ylabel("time")
    plt.savefig((path + "TIMEImage.png"), dpi=400, bbox_inches='tight')
    # plt.show()
    plt.clf()
    leavg, lcase = zip(*sorted(zip(results_df.E_AVG, results_df.CASE)))
    e_avg_plot = plt.bar(lcase, leavg)
    plt.xlabel("case")
    plt.ylabel("e_avg")
    plt.savefig((path + "AVGImage.png"), dpi=400, bbox_inches='tight')
    # plt.show()
    plt.clf()
    lemean, lcase = zip(*sorted(zip(results_df.E_MEAN, results_df.CASE)))
    e_mean_plot = plt.bar(lcase, lemean)
    plt.xlabel("case")
    plt.ylabel("e_mean")
    plt.savefig((path + "MEANImage.png"), dpi=400, bbox_inches='tight')
    # plt.show()
    plt.clf()
    lcores, lcase = zip(*sorted(zip(results_df.CORES, results_df.CASE)))
    core_plot = plt.plot(lcase, lcores)
    plt.xlabel("case")
    plt.ylabel("cores")
