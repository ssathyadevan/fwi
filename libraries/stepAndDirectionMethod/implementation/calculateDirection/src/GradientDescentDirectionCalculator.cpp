    ...
    namespace inversionMethods
    {
        ...
        core::dataGrid2D &GradientDescentDirectionCalculator::calculateDirection(const core::dataGrid2D &chiEstimate, const std::vector<std::complex<double>> &)
        {
            ...
            for(int i = 0; i < numberOfGridPoints; ++i)
            {
                ...
                // As for 'e_i' we mean a mathematical vector pointing towards the i-th direction/variable/grid point with unitary length, so that adding it to
                // another vector only modifies the i-th entry.
